...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    ...
                    fit = function(X, y)
                    {
                      ...
                      return(list(class_probs = class_probs,
                    ...
                    for (cls in names(class_probs)) {
                      ...
                      cls_idx <- which(cond_probs$class == as.numeric(cls))
                      for (word in message_words) {
                        if (word %in% vocab) {
                          word_col <- which(colnames(cond_probs) == word)
                          if (length(word_col) > 0) {
                            log_prob <- log_prob + log(cond_probs[cls_idx, word_col][[1]])
                          }
                        }
                      }
